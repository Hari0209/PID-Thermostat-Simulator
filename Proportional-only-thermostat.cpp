...
float temp_calculator(float heater_energy, float temp_outside, float temp, float volume){
    ...
    float heat_loss=3*(temp-temp_outside); /*the equation is k(temp-temp_outside) where k is the insulation (larger k value less insulation so more heat loss) */
...
/*room temperature at the start of the program*/
    ...
    while (people_inside==true &&time<=360){
        error=favoured_temp-temp;

        if (temp<favoured_temp){
            potential=error;
            heater_power=15*potential;}/*4 is he strength of the heater*/
